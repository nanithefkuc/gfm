//! The derived queries over GF(2): every one a reader of a cached [`Ple`],
//! none an elimination of its own (the one-elimination invariant). The
//! bit-domain twin of [`crate::dense`]'s `derive`, with characteristic-two
...
impl SolveScratch {
    ...
    fn ensure(&mut self, rows: usize, cols: usize) -> &mut BitMatrix {
...
impl core::fmt::Debug for SolveScratch {
...
impl Ple {
    /// The determinant over GF(2): one for a full-rank square matrix, zero
    /// otherwise. The product of `U`'s unit diagonal is one exactly when
    /// every pivot is present, and permutation signs are one in
    /// characteristic two.
    ///
    /// # Panics
    ///
    ...
    ///
    /// The column permutation is undone first — RREF is not invariant under
    /// column permutation — then Gauss–Jordan back-substitution clears each
    /// pivot column upward. Pivots are one, so no row is scaled.
    ///
    /// # Panics
    ///
    /// Panics unless `out` is `m × n`.
    pub fn rref_into(&self, out: &mut BitMatrix) {
        assert_eq!(
            (out.rows(), out.cols()),
            (self.rows(), self.cols()),
    ...
    /// and zeros at every other free position — the same convention as the
    /// dense domain.
    ///
    /// # Panics
    ///
    /// Panics unless `out` is `n × (n - rank)`.
    pub fn kernel_into(&self, out: &mut BitMatrix) {
        ...
        for col in 0..(cols - rank) {
            let free = rank + col;
            for piv in (0..rank).rev() {
                // acc = lu[piv][free] XOR sum_{term>piv} lu[piv][term]·z[term].
                ...
                // Pivot is one, so z[piv] = acc.
                out.set(piv, col, acc);
            }
            out.set(free, col, true);
        }
        // x = Q·z: un-permute the solution entries.
    ...
    pub fn solve_into(
        ...
        // Consistency: the eliminated tail rows must vanish on the right.
        for row in rank..rows {
            if !ws.row_is_zero(row) {
    ...
    pub fn inverse_into(&self, out: &mut BitMatrix) -> Result<(), SolveError> {
